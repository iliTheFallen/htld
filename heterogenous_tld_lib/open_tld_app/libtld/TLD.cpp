...
TLD::TLD()
{
	...
	imBlurred         = NULL;
	ppHolder          = NULL;
	rng               = cv::RNG((uint64)cv::getTickCount());
#ifndef USE_HTLD
	gaussianFilter    = cv::createGaussianFilter(CV_8UC1,
		                                         cv::Size((int)(6 * 2) + 1, (int)(6 * 2) + 1),
												 2.0,
												 0.0,
...
#else
	memMgr->moveCurFrameToGPU(currImg.data, false, true);
	detectorCascade->detect(currImg, 
		                    hTLDMaster->getTLDObject(tldObjId)->getFastDetStr(),
							true);
#endif
