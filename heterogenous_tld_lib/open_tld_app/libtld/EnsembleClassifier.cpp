...
int EnsembleClassifier::calcFernFeaturePatch(const unsigned char *blurredImg, int windowIdx, int treeIdx) {
	...
	for(int i = 0; i < numFeatures; i++) {
		...
		int fp0 = blurredImg[bbox[0] + off[0]];
		int fp1 = blurredImg[bbox[0] + off[1]];
...
void EnsembleClassifier::calcFeatureVectorPatch(const unsigned char *blurredImg, int windowIdx, int *featureVector) {
		
	for(int i = 0; i < numTrees; i++)
		featureVector[i] = calcFernFeaturePatch(blurredImg, windowIdx, i);
}
